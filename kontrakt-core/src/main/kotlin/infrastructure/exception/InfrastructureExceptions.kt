 ...
 */
open class InfrastructureException(message: String, cause: Throwable? = null) : KontraktException(message, cause)

/**
 * Exceptions specific to the [infrastructure.registry.RuntimeHandleRegistry].
 */
sealed class RegistryException(message: String) : InfrastructureException(message) {

    /**
     * Thrown when a cycleId collision occurs.
     * This usually happens when different types or ClassLoaders map to the same ID.
     */
    class Collision(val key: String, val existing: String, val new: String) : RegistryException(
     ...
     */
    class MissingPayload(val key: String) : RegistryException("Missing payload for key: '$key'. Check Discovery phase.")
}