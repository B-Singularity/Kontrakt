...
import stage.canonicalization.material.representation.FrozenMetadataAvailability
 ...
 *
 * This is the Level 1 frozen object-records representation for the raw fact
 * surface of a type.
 *
 * It is not:
 *
 ...
 *
 * - structured frozen records sequences; and
 ...
 * avoids per-read DTO assembly while the frozen records tree, sequence law, and
 ...
 * frozen records sequences or by lowering both records sequences and DTO shape
 ...
 *
 * A FrozenRawFactTable should expose one raw fact records or materialized fact
 * payload for each TypeReference in the owning FrozenMetamodelImage type index.
 *
 * Full records equality is broader than [reference].
 *
 * Rationale:
 *
 ...
 * - this records's equals/hashCode use the full frozen records payload.
 *
 * Backend-erasure law:
 *
 * This records, its nested sequences, and [materializedFacts] must not store or
 * retain:
 *
 * - KType;
 * - KClass;
 ...
 *
 * This records does not perform runtime deep scanning of [materializedFacts].
 *
 * Rationale:
 *
 ...
 * - frozen records constructors accept only adapter-neutral metamodel-domain
 *   values;
 * - frozen packages do not import backend-native APIs;
 * - architecture tests reject backend handle reachability;
 * - image publication validation checks cross-records continuity.
 *
 * Vertical continuity law:
 *
 * At issue time this records validates the local continuity that is available
 * without a FrozenMetamodelImage:
 *
 * - materializedFacts belongs to the same [reference] identity surface;
 * - every constructor records is owned by this [reference];
 * - every property records is owned by this [reference].
 *
 ...
 *
 * Type-level [annotationRecords] are attached to this raw fact records, but the
 * current FrozenAnnotationRecordKey does not yet carry an explicit owner type
 * surface.
 *
 * Therefore this records cannot locally prove that each annotation records belongs
 * to [reference].
 *
 ...
 * Availability law:
 *
 * [sourceAvailability] is records state.
 *
 ...
 * Equality law:
 *
 * Equality is structural over the full frozen records payload:
 *
 * - reference;
 * - constructorRecords;
 * - propertyRecords;
 ...
 * strategy globally. Do not introduce a local hash family in this records.
 */
class FrozenRawFactRecord private constructor(
    ...
    fun materializeFacts(): RawTypeFactsDTO {
         ...
         *
         * Later cuts may assemble the DTO from frozen records sequences or from
    ...
    companion object {
         ...
         * surface as this raw fact records.
         *
        ...
        private fun requireMaterializedFactsContinuity(
             ...
             *
             * Therefore this records cannot safely prove reference continuity
        ...
        /**
         * Validates that constructor records are owned by this raw fact records's
         * TypeReference.
         *
        ...
        /**
         * Validates that property records are owned by this raw fact records's
         * TypeReference.
         *
         * This is local parent/child continuity and does not require access to a
         * FrozenMetamodelImage.
