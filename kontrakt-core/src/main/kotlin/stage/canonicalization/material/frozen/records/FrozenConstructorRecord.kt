...
/**
 * Frozen constructor records.
 *
 * This is the Level 1 frozen object-records representation for one constructor
 ...
 * ```
 *
 * Full records equality is broader than key equality.
 *
 * Rationale:
 *
 ...
 * key.
 *
 * For every parameter records:
 *
 * ```text
 * parameterRecord.key.ownerConstructorKey == key
 * ```
 *
 * This records validates that continuity at issue time because the parent key is
 ...
 * records, but the current FrozenAnnotationRecordKey does not yet carry an
 * explicit owner-constructor key.
 *
 * Therefore this records cannot prove constructor-annotation ownership locally.
 *
 ...
 * records keys gain an explicit owner surface or when annotation records are
 * lowered into owner-addressed tables.
 *
 * Availability law:
 *
 * [declarationAvailability] and [sourceAvailability] are records state.
 *
 ...
 * Backend-erasure law:
 *
 * This records and its nested sequences must not store:
 *
 ...
 *
 * The records does not perform runtime "is lowered" self-attestation checks.
...
class FrozenConstructorRecord private constructor(
    ...
    companion object {
        ...
        /**
         * Validates that every nested parameter records points back to this
         * constructor key.
         *
         * This is the strongest ownership check this records can perform locally.
         *
