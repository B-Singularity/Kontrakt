...
class BooleanGenerator(
    ...
    override fun generate(context: GenerationContext): Boolean = fixedValue ?: context.seededRandom.nextBoolean()

    override fun generateEdgeCases(context: GenerationContext): List<Boolean> =
        if (fixedValue != null) listOf(fixedValue) else listOf(true, false)
     ...
     */
    override fun generateInvalid(context: GenerationContext): List<Any?> = if (fixedValue != null) listOf(!fixedValue) else emptyList()
}
...
class EnumGenerator<T : Enum<T>>(
    ...
    override fun generate(context: GenerationContext): T = constants.random(context.seededRandom)
