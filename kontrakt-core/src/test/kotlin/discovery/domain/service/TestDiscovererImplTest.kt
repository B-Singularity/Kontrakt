...
class TestDiscovererImplTest {
    ...
    @Test
    fun `discover - creates ContractAuto specs for interface implementations`() =
    ...
    @Test
    fun `discover - creates UserScenario specs for KontraktTest classes`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `discover - merges duplicate specs into a single one with combined modes`() =
        runTest(testDispatcher) {
            coEvery { scanner.findAnnotatedInterfaces(any(), Contract::class) } returns listOf(MyContract::class)
    ...
    @Test
    fun `dependency resolution - correctly identifies mocking strategies`() =
        runTest(testDispatcher) {
            coEvery { scanner.findAnnotatedInterfaces(any(), any()) } returns emptyList()
    ...
    @Test
    fun `determineMockingStrategy - handles Abstract Class dependencies correctly`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `createSpecificationForClass - handles DependencyMetadata creation failure`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `discover - ignores classes without primary constructor`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `discover - handles targets where interface is passed as class target`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `discover - logs warning when contract implementation is invalid`() =
        runTest(testDispatcher) {
            coEvery { scanner.findAnnotatedInterfaces(any(), Contract::class) } returns listOf(MyContract::class)
    ...
    @Test
    fun `discover - gracefully ignores invalid DataContract classes`() =
        runTest(testDispatcher) {
    ...
    @Test
    fun `createSpecificationForClass - handles null simpleName (UnnamedClass fallback)`() =
        runTest(testDispatcher) {
