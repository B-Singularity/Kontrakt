...
class TestSpecificationTest {
    ...
    private val dummyTarget: DiscoveredTestTarget = DiscoveredTestTarget.create(
        kClass = String::class,
        displayName = "String",
        fullyQualifiedName = "kotlin.String"
    ).getOrThrow()

    private val dummyDependency : DependencyMetadata = DependencyMetadata.create(
        name = "engine",
        type = List::class
    ).getOrThrow()
    ...
    fun `should create successfully with valid arguments`() {
        val result = TestSpecification.create(
            target = dummyTarget,
