...
* **Payload Insight:** * `capabilityDemotions` is sorted by `EntropyTargetKey` to ensure diagnostics are deterministic
  even if `DeclarationIndex` changes.
    * `truncated`: `false`
...
3. **Step 52:** Cycle detected. **Rollback** to Frame A.
    * **State Restoration:** Stack, Budget, Cache Log restored.
    * **Step Counter:** Remains **52** (Strictly Monotonic. It is explicitly excluded from the snapshot to prevent
      CPU-DoS via infinite rollback loops).
...
2. **Mapping:** Extract unsigned 64-bit Int `u` using bitwise masking (`& 0xFFL`). Compute modulo:
   `Long.remainderUnsigned(u, 2592000000L)`.
