...
| `D049` | **Alternative Hypotheses** | `C / R`       | Keeps multiple plausible causes separate when compiler-internal or realization evidence is insufficient to isolate one cause. | Contract and State-Machine judgments already have explicit authority and applicable material, so they do not require alternative hypotheses. In diagnostic investigation, this characteristic prevents a candidate explanation from being promoted into fact before the available evidence isolates it. |
...
| `D152` | **Adaptive Evidence Acquisition**        | `C / R`   | Optional diagnostic enrichment can change its target, depth, location, or resolution when an anomaly or new diagnostic hypothesis justifies further observation. Authoritative Contract or State-Machine meaning and required Diagnostic Evidence do not depend on this adaptive acquisition. | `D094 Triggerability` starts deeper capture. Adaptive acquisition changes what is observed after the trigger rather than merely turning a fixed recorder on; it remains an implementation-side enrichment technique rather than a source of Contract meaning. |
...
may enrich an investigation, but it cannot establish, revise, or complete the authoritative meaning being diagnosed.
