...
obligation. This is a correctness constraint on system allocation. It does not command one physical subsystem; a design
can satisfy the obligation either by close placement or by an explicit mechanism that preserves a legal coherent
observation across separated systems.
...
distinct questions. They are not required to become one universal lifecycle enum, but an implementation must not
collapse
them when that would change legality. Material can cease to be current while an existing reader can still legally
observe
it, can remain physically retained while no new request may use it, or can be reusable within one compiler lifetime
while
being unsuitable for persistence across sessions.

Different retention lifetimes therefore form a system-allocation pressure. A short-lived scratch or request-local result
must not acquire a longer retention, persistence, compatibility, or recovery obligation merely because it shares a
physical subsystem with longer-lived material. Conversely, assigning every lifetime distinction to a different physical
system is not required when that would duplicate state or add coordination without preserving a distinct legal boundary.
...
different systems when their coherence, lifetime, cost, traffic, replacement, resource, or failure domains differ,
provided that requesters observe the same legal protocol guarantees rather than those systems' internal machinery.
...
required by reusable material and cross-responsibility consumption.
...
Integrity, authenticity or provenance trust, representation compatibility, and current validity are distinct questions.
A content digest can establish that bytes match the claimed content without proving that the material came from an
allowed source, that its representation is compatible with the current producer, or that its determining basis remains
current. An implementation may perform several checks in one physical stage, but it must not collapse their meanings
into
one generic `valid` predicate when those distinctions affect legality or failure handling.

Failure and trust containment are system-allocation constraints. A boundary can be split when isolation, replacement, or
external execution justifies the cost, but the split must not allow an unqualified failure domain to contaminate
ordinary
successful state. Physical fusion is likewise legal only when it does not erase a required trust or qualification
boundary.

Reuse validity is not proof that the original producer was correct. A result can be perfectly reusable according to its
dependency metadata and still have been produced by a buggy analysis or transformation. Verification, translation
...
Hidden execution state cannot silently change that result. Worker scheduling, iteration order, cache presence, retained
state, locale, mutable global state, or previous execution history are examples of implementation conditions that must
not become undeclared determinants.

This section does not transfer Contract semantic determinism into ADR-0075. It applies the project-wide determinism
requirement to compiler-produced material and to the allocation machinery that mediates, retains, reuses, or recomputes
that material. Physical placement must not create a new determinant merely because responsibilities happen to share
memory, scheduling, storage, or a cache.

For the same applicable legal basis, alternative legal execution paths must expose the same approved meaning. A clean
recomputation, an in-memory reuse path, a persisted reuse path, different worker schedules, or a different physical
fusion of mediation responsibilities can change cost and execution history without changing what the requester is
legally allowed to observe.

Any compiler-specific condition that is allowed to change such a result must enter through an explicit producer-owned
basis appropriate to that result family. Legal nondeterministic input, if a future subsystem genuinely needs it, cannot
remain ambient. Time, randomness, runtime profile, or external capability information must enter through a declared form
that makes the resulting validity and reuse relation explicit.

Determinism constrains the exposed result, not the scheduling algorithm. Independent work can still run in parallel,
physical completion order can vary, adaptive algorithms can be used, and storage layout can change, provided that the
resulting legal observations satisfy the producer's declared guarantee.

The same rule constrains deep dependency machinery. A graph can be large or dynamically discovered when that provides
real value, but the owning system must still be able to explain the declared basis that can affect the exposed result.
Graph reachability, cache history, or incidental discovery order must not silently become the source of meaning.
...
them after preserving the Contract and determinism constraints defined by their owning boundaries.

Subsystem allocation follows a constrained decision order rather than one universal placement rule. Contract authority,
legal observation, explicit basis, and non-amplification first define what allocation is semantically legal.
Deterministic observation then requires the same applicable legal basis to produce the same approved meaning across
different schedules, cache outcomes, retention paths, and physical realizations. Coherence and failure or trust
containment further constrain which separations and combinations remain correct.

Only inside that legal space do operational allocation criteria choose among alternatives. Lifetime and retention ask
whether one placement forces unrelated state to share a lifetime, persistence, or reclamation obligation. Execution and
traffic ask whether frequency, latency sensitivity, material size, copying, blocking, synchronization, or boundary
crossing justify fusion or separate paths. Replacement and evolution ask whether responsibilities that should change
independently are being forced into one lockstep version or rollout domain. Resource and scaling ask whether CPU,
memory,
I/O, parallelism, or scaling pressure can be managed more effectively with a different placement.

These criteria can point in different directions. No criterion by itself determines the topology. Coherence can favor
close placement while replacement favors separation; the legal answer can be logical separation with physical proximity
or an explicit coherence mechanism. Traffic can favor fusion while trust requires isolation; the legal answer must
preserve the trust boundary even if the implementation makes that boundary inexpensive.

The cost of splitting includes routing, boundary crossings, synchronization, duplicated state, translation, copying,
qualification, and coherence machinery. The cost of fusion includes wider change and failure blast radius, shared-state
contention, lifetime coupling, replacement rigidity, reduced independent scaling, and accidental authority coupling.
Later architecture work should compare those costs for the actual workload rather than assume that either maximal
fragmentation or maximal consolidation is inherently superior.

Physical storage remains free to follow measured access patterns. Primitive tables, slabs, arenas, or snapshot-like
structures are all possible realizations. Logical responsibilities can be physically fused, and one logical relation can
use separate control and bulk-material paths, provided that physical optimization does not alter authority, protocol
meaning, or determinism.

Local dependency capture can likewise be dynamic or explicit, and persistent material can use any compatible format that
respects the producer boundary. Deep graphs can be chosen where their semantic or computational benefit justifies their
coordination and reasoning cost. They are not prohibited, but they do not become the default compiler-wide architecture
merely because a query or incremental engine can represent them.
...
The fourth is the concrete decomposition of mediation responsibilities. The allocation law is now substantially closed:
Contract authority and determinism constrain the legal space; coherence and failure or trust containment constrain
correctness; protocol containment and graph proportionality constrain topology; lifetime and retention, execution and
traffic, replacement and evolution, and resource and scaling guide physical placement. The remaining question is which
of requirement handling, protocol resolution, acquisition, qualification, comparison, retention, and delivery are
independent logical responsibilities for the actual result families and which are merely implementation steps.
...
The seventh is the relationship with ADR-0074. Unavailable, corrupt, unsupported, cancelled, internally failed, or
qualification-failed compiler results must have one consistent unsuccessful-result boundary before retention and
incremental recovery can be considered closed.

---

## 25. Consequences

This structure lets Kontrakt reuse compiler knowledge without turning cache, query topology, retained bytes, physical
placement, or one compiler-wide dependency graph into a second source of meaning. It also prevents each requesting
responsibility from managing producer lifecycle, comparison, qualification, and retention on its own.
...
observation without exposing producer-private construction state. Physical optimization is allowed only after Contract
authority, legal basis, and deterministic observation remain intact.
...
mechanisms, physically fuse logical responsibilities for hot-path efficiency, or split control and bulk-material paths
when their traffic characteristics differ. None of those choices transfers authority or permits execution history to
become a hidden determinant.

Different retention lifetimes, replacement cycles, failure domains, and resource or scaling pressures can therefore be
handled without forcing every compiler responsibility into one state-management model. The same freedom allows later
profiling and workload evidence to correct physical boundaries that prove too fragmented or too broad while preserving
the logical protocol relation.
...
leaving V2 free to adopt different incremental, persistence, scheduling, or distributed techniques behind those
boundaries.

This ADR remains Proposed. The next work is to decompose the protocol-mediation responsibilities against the actual
result families, then continue the qualification and preservation review without reopening the allocation principles
unless a concrete counterexample violates Contract authority, determinism, coherence, or trust.# ADR-0075:
Compiler-Produced Material and Knowledge, Protocol-Mediated Consumption, Validity, Reuse, and Incremental Boundaries
...
This ADR remains Proposed. The next work is to decompose the protocol-mediation responsibilities against the actual
result families, then continue the qualification and preservation review without reopening the allocation principles
unless a concrete counterexample violates Contract authority, determinism, coherence, or trust.